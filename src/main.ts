...
import { DEFAULT_SETTINGS, ICONS, SERVICES_INFO, TRANSLATOR_VIEW_ID } from "./constants";
...
export default class TranslatorPlugin extends Plugin {
	...
	async onload() {
		...
		this.message_queue = rateLimit(
			this,
			5,
			3000,
			true,
			default_timeout,
			(text: string, timeout: number = default_timeout, priority: boolean = false) => {
		...
		if (models_data) {
			...
			fasttext_data.set(
				JSON.parse(this.app.loadLocalStorage("fasttext")) || {
					binary: undefined,
					models: undefined,
					version: undefined,
				},
			...
			bergamot_data.set(
				JSON.parse(this.app.loadLocalStorage("bergamot")) || {
					binary: undefined,
					models: undefined,
					version: undefined,
				},
		 ...
		 */
		loaded_settings = nested_object_assign(
			DEFAULT_SETTINGS,
			loaded_settings ? loaded_settings : {},
			new Set(set_if_exists),
		) as TranslatorPluginSettings;
		...
		const commands: Array<CommandI> = [
			...
			},
			{
				...
				callback: () => {
					...
					(<HTMLTextAreaElement> most_recent_view.view.containerEl.find(".translator-left-column").children[1]
						.children[0]).focus();
				},
			},
		];
								...
								if (output.status_code === 200) {
									settings.update((x: TranslatorPluginSettings) => {
										if (!x.last_used_target_languages.contains(language)) {
											...
											x.last_used_target_languages = x.last_used_target_languages.filter(x =>
												x !== language
											);
											x.last_used_target_languages = [language, ...x.last_used_target_languages];
										}
	...
	async activateTranslatorView() {
		...
		if (loaded_settings.target_language_preference === "last") {
			target_language = loaded_settings.last_used_target_languages.last() ||
				loaded_settings.default_target_language;
		} else if (loaded_settings.target_language_preference === "specific") {
