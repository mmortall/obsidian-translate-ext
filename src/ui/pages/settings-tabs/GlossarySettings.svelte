<script lang="ts">
  ...
  function sortTable() {
    if (sort_direction >= 0) {
      ...
      if (!descending)
        glossary_pair = glossary_pair.sort((a, b) =>
  ...
  function filterTable() {
    ...
    const indexes: Set<number> = new Set(
      left_matches
        ...
        .filter((x) => x >= 0)
        .concat(
          ...right_matches.map((x, i) => (x ? i : -1)).filter((x) => x >= 0),
        )
        .sort((a, b) => a - b),
    );
  ...
  onDestroy(() => {
    ...
    plugin.app.vault.adapter.write(
      `${plugin.app.vault.configDir}/plugins/translate/glossary.json`,
      JSON.stringify(local_glossaries, null, "\t"),
    );
    glossary.dicts = local_glossaries;
    for (let key in glossary.dicts) {
      glossary.replacements[key] = new RegExp(
        glossary.dicts[key].map((item) => item[0]).join("|"),
  ...
  function addRow() {
    new_row[0] = new_row[0].replace(/^(?:&nbsp;|\s)+|(?:&nbsp;|\s)+$/gi, "");
    new_row[1] = new_row[1].replace(/^(?:&nbsp;|\s)+|(?:&nbsp;|\s)+$/gi, "");

    if (!new_row[0] || !new_row[1]) return;

    let duplicate_row = glossaries[language_pair].find(
      (row) => row[0] === new_row[0],
    );
    let reverse_duplicate_row: string[];
    if ($settings.glossary_bidirectional)
      reverse_duplicate_row = glossaries[reverse_language_pair].find(
        (row) => row[0] === new_row[1],
      );
    ...
    if (duplicate_row || reverse_duplicate_row) {
      ...
      if (duplicate_row && duplicate_row[1] === new_row[1]) {
        plugin.message_queue(
          `Entry (${new_row[0]}, ${new_row[1]}) already exists in ${src_language} → ${tgt_language} glossary`,
          5000,
        );
        return;
      }
      ...
      new ConfirmationModal(
        ...
        `Found a term that already exists in the glossary. Do you wish to <b>overwrite</b> it?<br>
          ...
          !duplicate_row
            ? ""
            : `<div class="translator-confirm-glossary"><div>${src_language} ⭢ ${tgt_language}</div>
					<div>(${duplicate_row[0]}, <span class="translator-confirm-glossary-change">${duplicate_row[1]}</span>) ⭢
					(${new_row[0]}, <span class="translator-confirm-glossary-change">${new_row[1]}</span>)</div></div>`
        }<br>
				${
          !reverse_duplicate_row
            ? ""
            : `<div class="translator-confirm-glossary"><div>${tgt_language} ⭢ ${src_language}</div>
        ...
        async () => {
          if (duplicate_row)
            glossaries[language_pair].splice(
              glossaries[language_pair].indexOf(duplicate_row),
              1,
            );
          if ($settings.glossary_bidirectional && reverse_duplicate_row)
            glossaries[reverse_language_pair] = glossaries[
              reverse_language_pair
            ].filter((row) => row[0] !== reverse_duplicate_row[0]);

          glossaries[language_pair] = [
            ...glossaries[language_pair],
            [...new_row],
          ];
          if ($settings.glossary_bidirectional)
            glossaries[reverse_language_pair] = [
              ...glossaries[reverse_language_pair],
              [new_row[1], new_row[0]],
            ];
          new_row = ["", ""];
  ...
  function deleteRow(row) {
    glossaries[language_pair] = glossaries[language_pair].filter(
      (r) => r != row,
    );
    updateGlossary();
    if ($settings.glossary_bidirectional)
  ...
  function updateRow(row) {
    ...
    if (row[0] === old_row[0] && row[1] === old_row[1]) {
      glossaries[language_pair][glossaries[language_pair].indexOf(row)] =
        old_row;
      return;
    }
    ...
    if (old_row[0] !== row[0]) {
      let duplicate_rows = glossaries[language_pair].filter(
        (r) => r[0] === row[0],
      );
      if (duplicate_rows.length > 1) {
        duplicate_row = duplicate_rows.find((r) => r[1] !== old_row[1]);
        if (!duplicate_row) {
          glossaries[language_pair] = glossaries[language_pair].map((r) =>
            r === row ? old_row : r,
          );
          plugin.message_queue(
            `Entry (${row[0]}, ${row[1]}) already exists in ${$all_languages.get(source_language)} → ${$all_languages.get(target_language)} glossary`,
            5000,
          );
          return;
        }
      ...
      let reverse_duplicate_rows = glossaries[reverse_language_pair].filter(
        (r) => r[0] === row[1],
      );
    ...
    if (duplicate_row || reverse_duplicate_row) {
      ...
      new ConfirmationModal(
        ...
        async () => {
          ...
          if (reverse_duplicate_row) {
            glossaries[reverse_language_pair] = glossaries[
              reverse_language_pair
            ].filter((r) => r !== reverse_duplicate_row);
            glossaries[reverse_language_pair] = glossaries[
              reverse_language_pair
            ].map((r) =>
              r[0] === old_row[1] && r[1] === old_row[0] ? [row[1], row[0]] : r,
            );
          }
          glossaries[language_pair] = glossaries[language_pair].map((r) =>
            r === old_row ? row : r,
          ...
          plugin.message_queue(
            duplicate_row && reverse_duplicate_row
              ? "Overwritten entries"
              : "Overwritten entry",
          );
          updateGlossary();
      ...
      glossaries[reverse_language_pair] = glossaries[reverse_language_pair].map(
        (r) =>
          r[0] === old_row[1] && r[1] === old_row[0] ? [row[1], row[0]] : r,
      );
    }
...
<div class="translator-flex-column-element markdown-rendered">
  ...
  >
    <Toggle
      slot="control"
      value={$settings.glossary_bidirectional}
      onChange={(value) => ($settings.glossary_bidirectional = value)}
    />
  </SettingItem>
  ...
  <div class="translator-glossary-buttons">
    ...
    />
    <div
      class="translator-filter-input-icon"
      on:click={() => (filter_text = "")}
    >
      <Icon icon="cross" />
    </div>
  </div>
  ...
  <table bind:this={table} class="translator-table">
    <thead>
      <tr>
        <th
          class="translator-table-wide-col"
          on:click={() => {
            sort_direction =
          ...
          <span class="translator-table-cell-header">
            ...
            {#if sort_direction !== -1 && sort_direction < 2}
              <span class="translator-table-cell-header-icon">
                <Icon
                  icon={`chevron-${sort_direction === 0 ? "down" : "up"}`}
                />
              </span>
            {/if}
          </span>
        ...
        <th
          class="translator-table-wide-col"
          on:click={() => {
            sort_direction = sort_direction < 2 ? 2 : 5 - sort_direction;
          }}
        >
          <span class="translator-table-cell-header">
            Target
            {#if sort_direction !== -1 && sort_direction > 1}
              <span class="translator-table-cell-header-icon">
                <Icon
                  icon={`chevron-${sort_direction === 2 ? "down" : "up"}`}
                />
              </span>
            {/if}
          </span>
        ...
        <tr>
          {#each row as cell, i}
            <td
              ...
              bind:innerHTML={cell}
              on:focusin={() => {
                old_row = JSON.parse(JSON.stringify(row));
              }}
              on:focusout={() => {
                updateRow(row);
              }}
              on:keydown={disableLinebreaks}
            ></td>
          {/each}
    ...
    <div slot="control" class="translator-flex-column-element">
      <Dropdown
        ...
        onChange={async (value) => {
          ...
          sync_state = null;
          translator =
            await plugin.reactivity.getTranslationService(sync_service);
        }}
      />
      <ToggleButton
        text="Sync glossary"
